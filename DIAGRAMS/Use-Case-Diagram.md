...
An editable draw.io copy sits beside this file: [`CAMS-Use-Case-Diagram.drawio`](CAMS-Use-Case-Diagram.drawio). One page, 41 module boxes, 202 use cases, with `<<include>>` and `<<extend>>` arrows. Each box carries a single actor standing outside it on the left. Open it at [app.diagrams.net](https://app.diagrams.net) with **File > Open From > Device**.
