...
4. **Teacher monitoring** — the authenticated web dashboard connects and is placed in `TeachersGroup` automatically; `ReceiveScreenFrame` events update live cards while `StudentConnected` / `StudentDisconnected` events add and remove cards.
