...
flowchart TD
    ...
    C --> A{Valid active student and safe workstation assignment?}
    A -- No --> D[Reject login and audit denial]
    A -- Yes --> LS[Create or safely assign station; create or resume LabSession]
...
3. Browser Student login does not create an assigned-workstation monitoring session. The CLIENT path submits its machine name and automatically creates or safely reassigns its workstation profile; conflicting active use is rejected.
4. Active teachers can monitor and control all connected student clients and use lab-wide pause/resume/end actions. Explicitly shared `/Admin/...` actions provide global account, class, roster, workstation and policy management. Individual session actions, older `/Teacher/...` management pages, and analytics/record queries retain their teacher or adviser/class checks. Admin-only administration remains restricted.
