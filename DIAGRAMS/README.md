...
| --- | --- | --- |
| [`CAMS-Use-Case-Diagram.drawio`](CAMS-Use-Case-Diagram.drawio) | UML use case | 41 module boxes, 219 use cases, 3 actors, one page |
...
| File | Contents |
| --- | --- |
| [`Use-Case-Specifications.md`](Use-Case-Specifications.md) | A written specification for all 219 use cases, ten fields each. 7,082 lines |
...
Each specification follows the ten fields the course handout sets out: **use case name, purpose, actors, input parameters, output parameters, pre-condition, post-condition, successful scenario, exception scenario, additional remarks.**
...
Every use case is named after the function that implements it. An ellipse reading `CREATE STUDENT` is `AdminController.CreateStudent`; one reading `LOGIN ASYNC` is `AuthenticationService.LoginAsync`, used where the behaviour has no endpoint of its own. The module caption names the declaring type. No use case is a bare noun: where the function name is a single word the caption carries the verb the action performs, so `AdminController.Teachers` reads `VIEW TEACHERS`.
