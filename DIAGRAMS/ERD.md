...
- [`CAMS-Class-Model.drawio`](CAMS-Class-Model.drawio) - the UML class model, which is a different document from the three above: it describes the *code* rather than the database. Twenty-three classes across four layers, with attributes typed as C# declares them, operation signatures, realizations of the service interfaces and associations taken from navigation properties. It is generated from `Server/Models` and the service files, so `Admin` correctly has no association - the C# class holds `CreatedById` as a plain `int?` with no navigation property to follow, which is exactly the difference between a class model and an ERD.
