...
[![Release](https://img.shields.io/badge/Release-v2.18.4-emerald.svg)](https://github.com/ACTstudent/RemoteAcessMonitoringSoftware4sale/releases/latest)
...
- Normalized-domain website policies. Applications are monitored but never policed: CAMS does not close anything on the desktop, and application rules raise no violation on the workstation. A session HTTP/HTTPS proxy rejects blocked website connections in browsers using Windows proxy settings; policy refresh also disconnects matching filtered connections. Previous Windows proxy settings are restored at logout and recovered on the next launch after a crash. The updated student client must be deployed; custom proxies/VPNs and cached content require deployment controls. See [website enforcement and rollout](docs/improvements/BLOCKING-FIX.md).
