...
; address this file by a fixed name through releases/latest/download/, and those
; links must keep working across releases. The version lives in the metadata
; below instead.
...
SetupIconFile=Monitoring And Remote Access\Server\cams.ico
...
Filename: "{app}\{#MyAppExeName}"; WorkingDir: "{app}"; Flags: nowait; StatusMsg: "Starting the CAMS server..."
...
begin
  ...
  begin
    AdminAccountPage := CreateInputQueryPage(wpSelectDir,
      'Administrator Account', 'Create or recover the CAMS administrator',
      'Enter credentials for the local administrator. If this username already exists, the password is reset, the account is activated, and any lockout is cleared. On an upgrade, leave the password blank to keep existing credentials unchanged.');
...
procedure ClearInitialAdminEnvironment;
begin
...
begin
  ...
  begin
    ...
    if (not Exec(ExpandConstant('{app}\{#MyAppExeName}'), '--initialize-only',
      ExpandConstant('{app}'), SW_HIDE, ewWaitUntilTerminated, ResultCode)) or (ResultCode <> 0) then
    begin
      ClearInitialAdminEnvironment;
      RaiseException('CAMS database initialization failed. Setup could not create or update the administrator account.');
    end;
    ClearInitialAdminEnvironment;
  end
